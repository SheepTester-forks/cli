...
const didYouMean = async (npm, path, scmd) => {
  const bestCmd = cmdList
    ...
    .map(str => `    npog ${str} # ${npm.commands[str].description}`)
  ...
  const bestRun = Object.keys(scripts || {})
    ...
    .map(str => `    npog run ${str} # run the "${str}" package script`)

  const { bin } = pkg
  const bestBin = Object.keys(bin || {})
    .filter(cmd => leven(scmd, cmd) < scmd.length * 0.4)
    .map(str => `    npog exec ${str} # run the "${str}" command from either this or a remote npm package`)
